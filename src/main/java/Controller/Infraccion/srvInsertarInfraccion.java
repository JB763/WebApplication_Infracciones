 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvInsertarInfraccion", urlPatterns = {"/srvInsertarInfraccion"})
public class srvInsertarInfraccion extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int respuesta = srvInfraccion.getSrvInfraccionPort().insertarInfraccion(idInfraccion, descripcion, multa);

        if (respuesta == 1) {
            request.setAttribute("mensaje", "Infraccion insertada correctamente");
        } else {
            request.setAttribute("mensaje", "Error al insertar la infraccion");
        }

        RequestDispatcher rd = request.getRequestDispatcher("NuevaInfraccion.jsp");
