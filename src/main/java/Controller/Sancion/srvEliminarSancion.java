 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvEliminarSancion", urlPatterns = {"/srvEliminarSancion"})
public class srvEliminarSancion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String idSancion = request.getParameter("iD");

            SrvSancion_Service srvSancion = new SrvSancion_Service();
            int respuesta = srvSancion.getSrvSancionPort().eliminarSancion(idSancion);

            if (respuesta == 1) {
                request.setAttribute("mensaje", "Sancion eliminada con éxito");
            } else {
                request.setAttribute("mensaje", "Error al eliminar la sancion");
            }
