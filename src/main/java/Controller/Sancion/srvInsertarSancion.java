 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvInsertarSancion", urlPatterns = {"/srvInsertarSancion"})
public class srvInsertarSancion extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String fechaSancion = request.getParameter("txtFecha");
        ...
        int respuesta = srvSancion.getSrvSancionPort().insertarSancion(idSancion, fechaSancion, idInfraccion, idLicencia, idAgente);

        if(respuesta == 1){
            request.setAttribute("mensaje", "Sanción insertada correctamente");
        }else{
            request.setAttribute("mensaje", "Error al insertar la sanción");
        }
        RequestDispatcher rd = request.getRequestDispatcher("NuevaSancion.jsp");
