 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvInsertarPago", urlPatterns = {"/srvInsertarPago"})
public class srvInsertarPago extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int respuesta = srvPago.getSrvPagoPort().insertarPago(idPago, fechaPago, monto, idSancion);

        if (respuesta == 1) {
            request.setAttribute("mensaje", "Pago insertado correctamente");
        } else {
            request.setAttribute("mensaje", "Error al insertar el pago");
        }
        request.getRequestDispatcher("NuevoPago.jsp").forward(request, response);
    }
