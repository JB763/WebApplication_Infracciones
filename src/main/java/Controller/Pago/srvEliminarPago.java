 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvEliminarPago", urlPatterns = {"/srvEliminarPago"})
public class srvEliminarPago extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int respuesta = srvPago.getSrvPagoPort().eliminarPago(idPago);

            if(respuesta == 1){
                request.setAttribute("mensaje", "Pago eliminado con éxito");
            }else{
                request.setAttribute("mensaje", "Error al eliminar el pago");
            }
