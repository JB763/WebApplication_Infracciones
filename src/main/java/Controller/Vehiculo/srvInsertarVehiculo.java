 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvInsertarVehiculo", urlPatterns = {"/srvInsertarVehiculo"})
public class srvInsertarVehiculo extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            // Instanciar la clase Vehiculo
        SrvVehiculo_Service srvVehiculo = new SrvVehiculo_Service();
        // Llamar el método insertarVehiculo
        int respuesta = srvVehiculo.getSrvVehiculoPort().insertarVehiculo(idVehiculo, placa, marca, modelo);

        if(respuesta == 1){
            request.setAttribute("mensaje", "Vehiculo insertado correctamente");
        }else{
            request.setAttribute("mensaje", "Error al insertar el vehiculo");
        }
