 ...
 * @author JULIAN BRITO
 */
@WebServlet(name = "srvInsertarUsuario", urlPatterns = {"/srvInsertarUsuario"})
public class srvInsertarUsuario extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int respuesta = srvUsuario.getSrvUsuarioPort().insertarUsuario(idUsuario, usuario, password, correo, idAgente);

        if (respuesta == 1) {
            request.setAttribute("mensaje", "Usuario insertado correctamente");
        } else {
            request.setAttribute("mensaje", "Error al insertar el usuario");
        }
